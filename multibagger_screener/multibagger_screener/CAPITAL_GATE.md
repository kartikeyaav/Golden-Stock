...
| 2026-09-07 | the **`nse_gap` cohort is excluded** from the gate, extended and legacy partitions (`scripts/gate_status.py: split_cohorts`) | 377 NSE coverage-gap names were added to `universe.csv` on 2026-09-07 (`PREREG_2026-09-07.md`). This gate was registered to judge *the entries the backtest validated*; those names are an EXPERIMENT justified by a backtest that is survivor-biased and explicitly not confirmation, and they have their own forward test. Admitting their signals would change the population a real-capital decision is measured on, mid-flight — in either direction. **This does not restart the sample (§8): no `nse_gap` signal had fired at the time of amendment, so the gate's collected data is byte-identical before and after.** A gap name promoted into an index at a future rebalance rejoins the gate under its index label. Guarded by `tests/test_gap_universe.py::test_gap_signals_never_enter_the_capital_gate_cohort`, verified able to go red. |
...
| 2026-09-12 | **DISCLOSURE — nothing changed.** Cohort, ruler, thresholds, sample rule and deadline all stand exactly as registered. | The scan crashed on every one of 18 attempts from 09-08 to 09-11 (a NaN industry on the 377 names added 09-07), and the two runs that did complete on 09-08 scanned at 0.1% price coverage. Three weekday sessions inside the judged window therefore have no scan at all. §5 reads n < 40 as a FREQUENCY finding about the trigger — an inference that is only sound if the scanner was awake, and until now nothing recorded whether it was. The denominator is now written per scan to `journal/scan_sessions.csv` (seeded from the committed `tags_state.json` history, live from 2026-09-12), and `gate_status.py` reports sessions observed, sessions missed, scans on degraded coverage, and the cohort rate per SCANNED week. First read: **32 of 35 weekday sessions observed (91.4%), 3 missed, 2.19 cohort signals per scanned week** — which projects to ~39 of the required 40 by the 12-01 maturity cutoff, i.e. the missed sessions are approximately the whole margin. This row moves no bar; it makes December's finding readable instead of confounded with downtime. |
| 2026-09-25 | **Ruler corrected (§8 "no mid-flight ruler swaps" procedure): the replay now stops a trade out on its own entry bar, and a data cut ends a week only on a Friday.** `journal_outcomes._plan_followed` passes `entry_day_stop=True`; `backtest.engine.week_end_flags` replaces the unconditional last-row week-end. | AUDIT 2026-09-22 F3 and F6, reproduced. The ruler fills at the next session's OPEN, but the engine processed exits before entries, so the rest of that bar — which happened after the fill — was never checked against the stop; and every mid-week data cut marked its last bar as a completed week, so a core lot could exit on a two-day "week" and a young signal could read CLOSED and count toward the sample at once. **The whole cohort was recomputed under the corrected ruler: 16 rows, one changed** — OPTIEMUS (EP, 2026-09-23) now stops on its entry day at −1.06R instead of gapping out the next morning at −1.21R; no status flipped between open and closed. Cohort mean on identical prices: **+0.410R → +0.420R**. Neutral to the collected data, so the sample does not restart. Guarded by `tests/test_measurement_fixes.py`, each test made to fail against the old behaviour first. |
| 2026-09-25 | **Benchmark identity corrected to the instrument this document registered.** `config.GATE.benchmark_symbol` MOMENTUM100 → **MIDSMALL** (Yahoo MIDSMALL.NS). | AUDIT 2026-09-22 F1, verified against the exchange listing: the ticker in use, MOM100.NS, is Motilal Oswal's plain **Nifty Midcap 100** ETF — not the Mirae Nifty MidSmallcap400 Momentum Quality 100 ETF that §4 and every label named. This is not a swap for a weaker comparator (§8): it restores the registered one, and it cuts the other way — from the cohort's first signal (2026-07-29) to 2026-09-25 MIDSMALL returned **−0.2%** against MOM100's **−2.2%**, so the mis-mapping had made condition 3 *easier*. The system still passes it (+5.2% signal basis). MOM100 stays refreshed as a labelled reference series so the correction is auditable. **Condition 3 remains directional only:** its system figure is a sum of independent signal results, not a cash-constrained portfolio (AUDIT F2), which this amendment does not fix. |
